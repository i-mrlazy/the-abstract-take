...
import { RATING_SCALE, normalizeScore, getQualityLabel } from '../../utils/rating';
import { Plus, Trash2, Edit, Save, Check, Sparkles, Send } from 'lucide-react';

interface WhatToWatchNextManagerProps {
  items: WhatToWatchNextItem[];
  onSaveItem: (item: WhatToWatchNextItem) => Promise<void>;
  onDeleteItem: (id: string) => Promise<void>;
}

export function WhatToWatchNextManager({
  items,
  onSaveItem,
  onDeleteItem,
}: WhatToWatchNextManagerProps) {
  ...
  const handleSave = async () => {
    ...
    const itemToSave: WhatToWatchNextItem = {
      ...
      posterUrl: posterUrl.trim() || 'https://images.unsplash.com/photo-1536440136628-849c177e76a1?q=80&w=800&auto=format&fit=crop',
        ...
        <div className="bg-white border border-gray-200/90 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="md:col-span-2">
              <label className="block text-xs font-mono uppercase text-gray-700 font-semibold mb-1">Title</label>
              <input
                type="text"
                value={title}
            ...
            <div>
              <label className="block text-xs font-mono uppercase text-gray-700 font-semibold mb-1">Mood Tag</label>
              <input
                type="text"
                value={moodTag}
            ...
            <div>
              <label className="block text-xs font-mono uppercase text-gray-700 font-semibold mb-1">Director</label>
              <input
                type="text"
                value={director}
            ...
            <div>
              <label htmlFor="whatToWatchScore" className="block text-xs font-mono uppercase text-gray-700 font-semibold mb-1">
            ...
            <div>
              <label className="block text-xs font-mono uppercase text-gray-700 font-semibold mb-1">Where To Watch</label>
              <input
                type="text"
                value={whereToWatch}
            ...
            <div className="md:col-span-3">
              <label className="block text-xs font-mono uppercase text-gray-700 font-semibold mb-1">Poster URL</label>
              <input
                type="text"
                value={posterUrl}
          ...
          >
            ...
            <div className="flex-1 min-w-0 flex flex-col justify-between">
              <div>
                ...
                <h4 className="font-serif font-bold text-sm text-gray-900 group-hover:text-[#008CFF] transition-colors mt-1.5">{item.title}</h4>
                <p className="text-[11px] text-gray-600 line-clamp-2 mt-0.5 leading-relaxed">{item.personalCommentary}</p>
              </div>
