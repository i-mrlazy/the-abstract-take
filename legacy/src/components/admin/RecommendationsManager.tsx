...
import { RecommendationList, RecommendationListItem, MediaType } from '../../types';
import { RATING_SCALE, normalizeScore, getQualityLabel } from '../../utils/rating';
import { Plus, Trash2, Edit, Save, ArrowLeft, MoveUp, MoveDown, Check, ListOrdered } from 'lucide-react';

interface RecommendationsManagerProps {
  lists: RecommendationList[];
  onSaveList: (list: RecommendationList) => Promise<void>;
...
export function RecommendationsManager({
  ...
  const [coverUrl, setCoverUrl] = useState('https://images.unsplash.com/photo-1536440136628-849c177e76a1?q=80&w=1200&auto=format&fit=crop');
  ...
  const handleStartNew = () => {
    ...
    setDescription('');
    setCoverUrl('https://images.unsplash.com/photo-1536440136628-849c177e76a1?q=80&w=1200&auto=format&fit=crop');
    setItems([
      {
        id: `item-${Date.now()}`,
        title: '',
                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => handleUpdateItem(idx, 'abstractScore', Number(e.target.value))}
        ...
        {lists.map((list) => (
          ...
          >
            <div>
              ...
              <h3 className="font-serif font-bold text-base text-gray-900 group-hover:text-[#008CFF] transition-colors">{list.title}</h3>
              <p className="text-xs text-gray-600 mt-1.5 line-clamp-2 leading-relaxed">{list.subtitle}</p>
            </div>
