...
import { getQualityLabel, normalizeScore } from '@/lib/utils/rating';
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <header className="space-y-4 mb-8">
          <div className="flex flex-wrap items-center gap-2">
            <span className="px-3 py-1 bg-gray-100 rounded-lg text-xs font-mono font-bold uppercase tracking-wider text-gray-800">
              {review.type}
            </span>
            <span className="text-xs font-mono text-gray-500">•</span>
            <span className="text-xs font-mono text-gray-600">{review.releaseYear}</span>
            <span className="text-xs font-mono text-gray-500">•</span>
            <span className="text-xs font-mono text-gray-600">Dir. {review.director}</span>
          </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100">
            <div className="flex items-center space-x-3">
              ...
              />
              <div>
                <p className="font-mono text-xs font-bold text-gray-900">{review.author.name}</p>
        ...
        <div className="bg-white border-2 border-gray-900/10 rounded-3xl p-6 sm:p-8 mb-10 shadow-sm">
          <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-6 pb-6 border-b border-gray-100 text-center sm:text-left">
            <div className="space-y-2">
              ...
              <p className="font-news text-base text-gray-700 leading-relaxed max-w-xl">
                {review.verdictText}
              </p>
            </div>
            ...
            <div className="space-y-3">
              ...
              <ul className="space-y-2">
                {review.pros.map((pro: string, idx: number) => (
                  <li key={idx} className="font-news text-sm text-gray-800 flex items-start space-x-2">
        ...
        <div className="my-8 p-6 bg-[#008CFF]/5 border-l-4 border-[#008CFF] rounded-r-2xl">
          <span className="text-[11px] font-mono font-bold tracking-widest text-[#008CFF] uppercase block mb-1">
            The Creator's Raw Take
          </span>
          <p className="font-serif italic font-bold text-lg sm:text-xl text-gray-950 leading-snug">
            "{review.myTake}"
          </p>
        </div>

        {/* Long Form Editorial Review */}
        <div className="font-news text-lg text-gray-800 leading-relaxed space-y-6 my-10 whitespace-pre-line">
          {review.longFormReview}
        </div>

        {/* Favorite Scene / Quote */}
        {review.favoriteQuote && (
          <div className="my-8 p-6 bg-gray-50 border border-gray-200 rounded-2xl flex items-start space-x-4">
            <Quote className="w-8 h-8 text-[#008CFF] shrink-0 opacity-40 mt-1" />
            <div>
              <span className="font-mono text-xs font-bold uppercase tracking-wider text-gray-500 block mb-1">
                Memorable Line
              </span>
              <p className="font-serif italic text-base text-gray-900">"{review.favoriteQuote}"</p>
            </div>
          </div>
        )}
        ...
        <footer className="mt-12 pt-8 border-t border-gray-200/80 space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-xs text-gray-500 mr-2">Genres:</span>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-xs text-gray-500 mr-2">Cast:</span>
