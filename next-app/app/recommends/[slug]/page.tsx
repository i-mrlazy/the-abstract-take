  ...
  return (
    <article className="py-8 md:py-12">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Collection Header */}
        <header className="space-y-4 mb-10 pb-8 border-b border-gray-200/80">
          <div className="flex items-center space-x-3">
            <span className="px-3 py-1 bg-[#008CFF]/10 text-[#008CFF] rounded-full text-xs font-mono font-bold uppercase tracking-wider">
              {list.category}
            </span>
          ...
          {list.subtitle && (
            <p className="font-serif italic text-lg sm:text-xl text-gray-700 leading-snug">
              {list.subtitle}
            </p>
          )}

          <p className="font-news text-base sm:text-lg text-gray-700 leading-relaxed max-w-3xl">
            {list.description}
          </p>
              ...
              <div className="flex-1 flex flex-col justify-between space-y-4">
                <div>
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    <div className="flex items-center space-x-2 text-xs font-mono text-gray-500">
                      ...
                      <span>•</span>
                      <span>{item.year || item.releaseYear}</span>
                      <span>•</span>
                  ...
                  <div className="mt-4 p-4 bg-gray-50/80 border-l-3 border-[#008CFF] rounded-r-xl">
                    <span className="text-[10px] font-mono font-bold tracking-widest text-[#008CFF] uppercase block mb-1">
                      Curator Commentary
                    </span>
                    <p className="font-news text-base text-gray-800 leading-relaxed">
                      "{item.curatorNote}"
                    </p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 pt-4 border-t border-gray-100 text-xs font-mono">
                  <div className="flex items-center space-x-2 text-gray-600">
                    ...
                    <span>Where to watch: <strong className="text-gray-900">{item.whereToWatch}</strong></span>
                  </div>

                  {item.reviewId && (
                    <Link
                      href={`/reviews/${item.reviewId}`}
                      className="inline-flex items-center space-x-1 text-[#008CFF] font-bold hover:underline"
                    >
