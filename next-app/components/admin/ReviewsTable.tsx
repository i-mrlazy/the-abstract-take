...
export function ReviewsTable({ initialReviews }: ReviewsTableProps) {
  ...
  const getScoreBadge = (score: number) => {
    ...
    return (
      <span
        className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg text-xs font-mono font-bold border ${colors.badgeBg} ${colors.badgeText} ${colors.border}`}
      >
        <span>{norm}/10</span>
  ...
  const handleDuplicate = async (id: string) => {
    setDuplicatingId(id);
    try {
      const res = await fetch(`/api/reviews/${id}/duplicate`, { method: 'POST' });
      if (res.ok) {
        const data = await res.json();
        if (data.review) {
          setReviews((prev) => [data.review, ...prev]);
          router.push(`/admin/reviews/${data.review.id}/edit`);
        }
      }
    } catch (err) {
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end space-x-1.5">
                        ...
                        <button
                          ...
                          className="p-2 bg-gray-50 hover:bg-blue-50 rounded-xl border border-gray-200 text-gray-600 hover:text-[#008CFF] transition-colors shadow-2xs cursor-pointer"
                          title="Live Preview"
                        >
                        ...
                        <button
                          ...
                          className={`px-2.5 py-1 rounded-xl text-[10px] font-mono font-bold border transition-colors shadow-2xs cursor-pointer ${
                            review.status === 'published'
                        ...
                        {/* Duplicate */}
                        <button
                          onClick={() => handleDuplicate(review.id)}
                          disabled={duplicatingId === review.id}
                          className="p-2 bg-gray-50 hover:bg-gray-100 rounded-xl border border-gray-200 text-gray-600 hover:text-gray-900 transition-colors shadow-2xs cursor-pointer disabled:opacity-50"
                        ...
                        <button
                          ...
                          className="p-2 bg-red-50 hover:bg-red-100 rounded-xl border border-red-200 text-red-600 transition-colors shadow-2xs cursor-pointer disabled:opacity-50"
                          title="Delete Review"
                        >
