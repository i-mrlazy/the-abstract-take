...
import { Download, Trash2, UserPlus, Mail } from 'lucide-react';
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.subscriber) {
        ...
        setNewEmail('');
        setMsg(data.message || 'Subscriber added successfully!');
        router.refresh();
      } else {
        setMsg(data.message || data.error || 'Failed to add subscriber');
      }
    } catch (err: any) {
