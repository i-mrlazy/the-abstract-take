...
export function ReviewsArchiveView({
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12 space-y-8 min-w-0">
      {/* Archive Header */}
      <header className="border-b border-gray-200/80 pb-6 min-w-0">
        {badge && (
      ...
      {/* Grid or Empty State */}
      {reviews.length === 0 ? (
        <div className="py-16 text-center bg-white border border-gray-200 rounded-3xl p-8 min-w-0">
      ...
      {totalPages > 1 && (
        <nav className="flex flex-wrap items-center justify-between gap-4 pt-8 border-t border-gray-200/80" aria-label="Pagination">
          {hasPrev ? (
            <Link
              href={`${basePath}?page=${currentPage - 1}`}
              className="inline-flex items-center space-x-1.5 px-4 py-2.5 bg-white border border-gray-200 rounded-xl text-xs font-mono font-bold text-gray-800 hover:border-[#008CFF] transition-colors shadow-2xs min-h-[44px]"
            >
              <ChevronLeft className="w-4 h-4" />
              <span>PREVIOUS</span>
            </Link>
          ) : (
            <div className="opacity-40 px-4 py-2.5 border border-transparent text-xs font-mono font-bold text-gray-400 min-h-[44px] flex items-center">
              PREVIOUS
            </div>
          )}
